...
public class BladeX extends JavaPlugin implements Listener {
    ...
    private void openSwordMenu(Player p) {
        // Eski çalışan yöntemle birebir aynı — String title, renk kodlu
    ...
    private ItemStack buildSword(String key, ConfigurationSection sec) {
        // Material config'den al, yoksa NETHERITE_SWORD
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        // Eski çalışan yöntemle birebir: title'dan "Kılıç Menü" ara
        ...
        // Sadece üst envantere (menüye) tıklamayı işle
    ...
    private void handleSwordSkill(Player p, String type, List<Player> nearby) {
        switch (type) {
            ...
            case "phantom" -> {
                ...
                nearby.forEach(t -> {
                    // 4 saniye elytra kilidi (orijinal 3sn + 1sn)
    ...
    private void handleBottomAction(Player p, String action) {
        switch (action) {
            case "SELF_PHANTOM_LOCK" -> {
                // Phantom gibi kendine uygula — 4 saniye
            ...
            case "SELF_INVINCIBILITY" -> {
                // 2.5 saniye tam koruma — Resistance V
