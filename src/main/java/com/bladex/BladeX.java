...
public class BladeX extends JavaPlugin implements Listener {
    ...
    /** Bilgi itemi — menünün altına, ortaya */
    ...
    private void handleSwordSkill(Player p, String type, List<Player> nearby) {
        switch (type) {
            ...
            // ── Enderman — 2.5 blok ileriye ısınla, katı blok kontrolü ───────
            case "enderman" -> {
                p.playSound(p.getLocation(), Sound.ENTITY_ENDERMAN_TELEPORT, 1f, 1f);
                p.getWorld().spawnParticle(Particle.PORTAL, p.getLocation().add(0, 1, 0), 30, 0.5, 1, 0.5, 0.1);

                // Baktığı yönde 2.5 blok ileriye güvenli konum bul
                Location dest = findSafeTeleportForward(p, 2.5);
                if (dest != null) {
                    p.teleport(dest);
                    p.getWorld().spawnParticle(Particle.PORTAL, dest.clone().add(0, 1, 0), 30, 0.5, 1, 0.5, 0.1);
    ...
    /**
     * Oyuncunun baktığı yönde maxDist blok ileriye güvenli ışınlanma noktası bulur.
     * Geçilmez blokların içine ışınlamaz; hava, odun (log/wood) ve örümcek ağı geçerlidir.
     */
    private Location findSafeTeleportForward(Player p, double maxDist) {
        ...
        // 0.5'er adımlarla ileriye bak, en uzak güvenli noktayı al
        Location lastSafe = null;
        for (double d = 0.5; d <= maxDist; d += 0.5) {
            Location check = origin.clone().add(dir.clone().multiply(d));
            // Ayak ve kafa bloklarını kontrol et
            Block feet = check.getBlock();
            Block head = check.clone().add(0, 1, 0).getBlock();
            if (isPassable(feet) && isPassable(head)) {
                // Altında zemin var mı veya yüksekliği yeterli mi kontrol et
                lastSafe = check;
            } else {
                // Katı blok — bu noktada dur, son güvenli konumu kullan
                break;
            }
        }
        if (lastSafe == null) return null;
        // Yaw/pitch koru
        lastSafe.setYaw(p.getLocation().getYaw());
        lastSafe.setPitch(p.getLocation().getPitch());
        return lastSafe;
    }
