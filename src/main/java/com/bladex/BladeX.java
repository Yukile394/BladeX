...
public class BladeX extends JavaPlugin implements Listener {
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        // Her turlu hareketi engelle — menuden hicbir sekilde item cikmaz/girmez
        e.setCancelled(true);

        // Sadece ust envanteri (menunun kendisi) isle
    ...
    /** WorldGuard: PVP flag DENY olan bolgede yetenekler calismasin */
    private boolean isInProtectedRegion(Player p) {
        try {
            ...
            // Sadece PVP=DENY olan bolgelerde engelle (ALLOW veya null ise serbest)
    ...
    private void handleBottomAction(Player p, String action, ConfigurationSection sec) {
        ...
        switch (action) {
            case "SELF_PHANTOM_LOCK" -> {
                ...
                p.sendTitle("", color("&#ff6600Elytra Kilidi! &l1.8 saniye."), 5, 36, 5);
                p.sendMessage(color(cfg("prefix") + "&#ff6600Elytra &l1.8 saniye kilitlendi."));
            }
            case "SELF_POWER_BOOST" -> {
                ...
                p.sendTitle("", color("&#0099ffGuc Yildizi! &l3 saniye."), 5, 60, 5);
                p.sendMessage(color(cfg("prefix") + "&#0099ffGuc Yildizi! &lDirenc + Hiz II, 3 saniye."));
            }
            case "SELF_INVINCIBILITY" -> {
                ...
                p.sendTitle("", color("&#00ee66Yenilmezlik! &l2.5 saniye."), 5, 50, 5);
                p.sendMessage(color(cfg("prefix") + "&#00ee66Yenilmezlik aktif! &l2.5 saniye hasar almayacaksin."));
            }
        }
