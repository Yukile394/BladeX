...
public class BladeX extends JavaPlugin implements Listener {
    ...
    // Açık kılıç menüsü inventory'lerini takip et (title karşılaştırması yerine)
    ...
    private void openSwordMenu(Player p) {
        ...
        // Güvenli size (9'un katı, 9-54 arası)
        if (size % 9 != 0) size = 36;
        if (size < 9) size = 9;
        if (size > 54) size = 54;
    ...
    public void onInventoryClick(InventoryClickEvent e) {
        ...
        // Sadece kendi takip ettiğimiz menüleri engelle
    ...
    // Menü kapanınca UUID'yi temizle
    @EventHandler
    ...
    private String getSwordDisplayName(String key) {
        ...
        if (sec != null) {
            return ChatColor.stripColor(color(sec.getString("display_name", key)));
        }
